...
local Material = {
	Close = Material("polaroid/close.png", "smooth", "noclamp"),
	Down = Material("polaroid/down.png", "smooth", "noclamp"),
	Ramp = Material("polaroid/ramp.png", "smooth", "noclamp"),
	Question = Material("polaroid/question.png", "smooth", "noclamp"),
	Label = Material("polaroid/label.png", "smooth", "noclamp"),
	Frame = Material("polaroid/frame.png", "smooth", "noclamp"),
}

local PANEL = {}

PANEL.Icon = Material.Question
PANEL.Title = PolaroidLANG("What are we going to do?")
PANEL.Desc = PolaroidLANG("please, select an action")

surface.CreateFont("incredible-gmod.ru/polaroid/title", {
    font = "Roboto",
    extended = true,
    size = 26,
    weight = 700,
...
surface.CreateFont("incredible-gmod.ru/polaroid/desc", {
    font = "Roboto",
    extended = true,
    size = 20,
    weight = 300,
...
function PANEL:Init()
	...
	self.Title = PolaroidLANG("What are we going to do?")
	self.Desc = PolaroidLANG("please, select an action")
	...
	hook.Add("Polaroid/LangsLoaded", self, function()
		...
		for i, elem in ipairs(self.TranslatedElements) do
			if IsValid(elem) and elem._LangPhrase then
				elem:SetText(PolaroidLANG(elem._LangPhrase))
			end
		end
	end)
	...
	self.WorkSpace.Paint = function(me, w, h)
		...
		local tw, th = SimpleText(self.Title, "incredible-gmod.ru/polaroid/title", w*0.5, h*0.45 + t_add, Colors.WhiteBG, CENTER, CENTER)
		SimpleText(self.Desc, "incredible-gmod.ru/polaroid/desc", w*0.5, h*0.45 + t_add + th, Colors.WhiteBG, CENTER, CENTER)
	end
...
function PANEL:CreateButtons(lang_phrase1, lang_phrase2, parent, ent)
	local btn = parent:Add("incredible-gmod.ru/polaroid/button")
	btn.DoClick = function()
		self:SetMode(ent, 2)
	end

	local btn2 = parent:Add("incredible-gmod.ru/polaroid/button")
	...
	btn:SetText( PolaroidLANG(lang_phrase1) )
	btn._LangPhrase = lang_phrase1

	btn2:SetText( PolaroidLANG(lang_phrase2) )
...
PANEL.ModeList = {
	...
	[2] = function(self, parent, ent)
		...
		self.Title = PolaroidLANG("Title editing..")
		self.Desc = PolaroidLANG("please, enter a new title")
		...
		local input = parent:Add("incredible-gmod.ru/polaroid/input")
		...
		input.btn:SetText(PolaroidLANG("OK"))
		input.btn._LangPhrase = "OK"
		input.btn.Paint = function(me, w, h)
			RoundedBox(0, 0, 0, w, h, input.CurrentColor)

			SimpleText(me:GetText(), "incredible-gmod.ru/polaroid/title", w*0.5, h*0.5, ok_color, CENTER, CENTER)
		end
	...
	[4] = function(self, parent, ent)
		...
		self.Title = PolaroidLANG("Picture Frame")
		self.Desc = PolaroidLANG("please, put a photo in me")
		self.IcoMult = 2.75
		self.IcoYAdd = 64
		self.TextYAdd = 64
...
vgui.Register("incredible-gmod.ru/polaroid/menu", PANEL, "EditablePanel")

local PANEL = {}

PANEL.Text = "me:SetText(text, font)"
PANEL.Font = "incredible-gmod.ru/polaroid/title"
...
vgui.Register("incredible-gmod.ru/polaroid/button", PANEL, "EditablePanel")
...
function PANEL:Init()
	...
	self:SetFont("incredible-gmod.ru/polaroid/title")
...
vgui.Register("incredible-gmod.ru/polaroid/input", PANEL, "DTextEntry")