...
function SWEP:Deploy()
    if SERVER and not (UserGroups.StaffRanks[self:GetOwner():GetUserGroup()] or self:GetOwner():Team() == FACTION_STAFF) then
        self:Remove()
    end
end
...
function SWEP:DrawWorldModel()
    if not UserGroups.StaffRanks[self:GetOwner():GetUserGroup()] and self:GetOwner():GetMoveType() == MOVETYPE_NOCLIP then return end
    self:DrawModel()
end