...
function AdminStick:OpenAdminStickUI(isright, target)
    ...
    if target:IsPlayer() then
        ...
        local spectate = AdminMenu:AddOption("Spectate: " .. target:Nick(), function()
            RunConsoleCommand("sam", "spectate", target:SteamID())
            AdminStick.IsOpen = false
        end)
        ...
        local desc2 = characterInfo:AddOption("Prompt Name Change", function()
            LocalPlayer():ConCommand('say /forcednamechange "' .. target:SteamID() .. '"')
            AdminStick.IsOpen = false
        end)
    ...
    elseif target:isDoor() then
        ...
        if target:getNetVar("faction") ~= nil then
            ...
            removeallfaction:SetIcon("icon16/door_in.png")
        end
        ...
        if target:getNetVar("owner", 0) > 0 then
            ...
            removeowner:SetIcon("icon16/door_in.png")
        end
    elseif target:GetClass() == "nut_storage" then
