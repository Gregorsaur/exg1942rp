...
function b:Flash(text, color, time, noAdjust, callback)
	...
	timer.Simple(time, function()
		...
		if callback then callback() end --Call callback
	end)
end
...
function b:GInflate(color, over)
	...
	function self:Paint(w,h)
		...
		if CurTime() >= (self.animStart+self.animDur) then --The end of the expansion animation
...
function te:SetError(err, ogCol)
	...
	local ogText = self:GetText()
	self:SetText(err)
	timer.Simple(1,function() --Reset text
