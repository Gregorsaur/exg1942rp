	...
	onRun = function(item)
		...
		local position = item.player:getItemDropPos()
			...
			if(item.FixLegs) then
				item.player:getChar():setData("leg_injury", nil)
				DISEASES.diseases["leg_injury"].effectC(item.player, item.player:getChar())
			end
		end
	end,
...
ITEM.functions.usef = { -- sorry, for name order.
	...
	onRun = function(item)
		...
		if (target and target:IsValid() and target:IsPlayer() and target:Alive()) then
			...
			if(item.FixLegs) then
				target:getChar():setData("leg_injury", nil)
				DISEASES.diseases["leg_injury"].effectC(target, target:getChar())
			end
			return true
		end
