...
PLUGIN.thinkTime = 600 --time between running disease functions
PLUGIN.chance = 0.5 --chance of a random disease happening every thinkTime
...
function diseaseEffects(client, disease, diseaseT)		
	...
	if(disTable) then --if the disease exists
		...
		if(iTime) then
			...
			/*local squaredDist = (disTable.spreadRange or 500)*(disTable.spreadRange or 500)
			if(disTable.spreadRange or disTable.spreadChance) then --if the disease is contagious
				local players = player.GetAll()
				
				for k, v in pairs(players) do
					if v != client and v:GetPos():DistToSqr(client) <= squaredDist then
						if(v:getChar() and !v:getChar():getData(disease)) then --if the player does not have the disease
							...
							if(roll <= disTable.spreadChance) then --rolls to see if the disease spreads
								-- if(char and !char:getData(disease)) then --only want to infect people who don't already have the disease.
									v:getChar():setData(disease, CurTime())
								-- end
							end
						end
					end
