-- "gamemodes\\1942rp\\plugins\\search\\cl_searchvgui.lua"
-- Retrieved by https://github.com/c4fe/glua-steal
netstream.Hook("searchUpdate", function(itemID)
	if (LocalPlayer().searchPanel) then
		LocalPlayer().searchPanel:InvRemoveItem(itemID)
	end
end)
...
PANEL = {}
	function PANEL:Init()
		...
		self.panels = {}
		LocalPlayer().searchPanel = self
	end
	
	function PANEL:InvRemoveItem(itemID)
		print("updating!")
				...
				for y, data in pairs(items) do
					...
					if (item and !IsValid(self.panels[item.id])) then
						...
						if (IsValid(icon)) then
							...
							icon.Paint = function(ic, ic_x, ic_y)
								icon:oldPaint(ic_x, ic_y)
								if (IllegalItems[item:getName()] != nil) then
									surface.SetDrawColor(255, 0, 0, 50)
									surface.DrawRect(0, 0, ic_x, ic_y)
								end
							end
		...
		if (self.slots[x] and self.slots[x][y]) then
			...
			if (itemTable.exRender) then
				...
				panel.ExtraPaint = function(self, x, y)
					if (panel.itemTable == nil) then
						panel:Remove()
						return
					end
					if (IllegalItems[itemTable:getName()]) then
						surface.SetDrawColor(255, 100, 100, 75)
			...
			panel.doRightClick = function(this)
				print(itemTable:getName())
				if (itemTable && IllegalItems[itemTable:getName()] != nil) then
					itemTable.player = LocalPlayer()
					local menu = DermaMenu()
					menu:AddOption("Confiscate", function()
						itemTable.player = LocalPlayer()
						netstream.Start("confiscateItem", itemTable.id)
...
vgui.Register("nutSearch", PANEL, "DFrame")
