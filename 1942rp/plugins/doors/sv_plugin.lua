...
function PLUGIN:LoadData()
	...
	for k, v in pairs(data) do
		...
		if (IsValid(entity) and entity:isDoor()) then
			entity:setNetVar("faction", v.factions)
...
function PLUGIN:SaveDoorData()
	...
	for k, v in pairs(doors) do
		...
		doorData.factions = v:getNetVar("faction")
...
function PLUGIN:CanPlayerAccessDoor(client, door, access)
	...
	if (string.find(door:GetNWString("factions"), string.sub(team.GetName(client:getChar():getFaction()), 1, 4))) then
		return true
	end
...
function PLUGIN:ShowTeam(client)
	...
	if (IsValid(entity) and entity:isDoor() and !entity:getNetVar("faction") and !entity:getNetVar("class")) then
		if (entity:checkDoorAccess(client, DOOR_TENANT)) then
			...
			if !(entity:GetNWString("factions") == " " or entity:GetNWString("factions") == "") then return end
