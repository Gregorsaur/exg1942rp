...
function PANEL:Init()
    ...
    function self.list:ReArrange()
      ...
      for k,v in pairs(children) do
        if #children % 2 != 0 then --If number is not even
          if k == #children then
    ...
    local function getEntryPanel()
      ...
      function fep:Paint(w,h) end --nopaint
      return fep
    end
    ...
    function self:AddTextEntry(placeholder)
      ...
      local fe = fep:Add("DTextEntry")
        fe:SetText(placeholder)
        function fe:Think()
                if fe:IsEditing() and fe:GetText() == placeholder then
                    fe:SetText("")
                end
                if not fe:IsEditing() and fe:GetText() == "" then
                    fe:SetText(placeholder)
                end
        end
      defaultLayout(fe)
  ...
  vgui.Register("nutForm", PANEL, "DScrollPanel")