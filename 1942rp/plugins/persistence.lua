...
if (SERVER) then
    nut.log.addType("persistedEntity", function(client, entity) return string.format("%s has persisted '%s'.", client:Name(), entity) end)
    nut.log.addType("unpersistedEntity", function(client, entity) return string.format("%s has removed persistence from '%s'.", client:Name(), entity) end)
    ...
    function PLUGIN:SaveData()
        ...
        for k, v in ipairs(self.entities) do
            if (IsValid(v)) then
                ...
                entData.NetVars = V:getNetVar("number")
